...
var pointRadius = 4;
var alphaChannel = 0.60;

function niceNumber(num) {
  num = num.toPrecision(4);
  try {
      //if (num != 0 && (Math.abs(num) >= 1e6 || Math.abs(num) <= 1e-6)) num = num.toExponential();
      var sOut = num.toString().replace("+", "");
      if (sOut.indexOf("e") > 0) {
...
wpd.ImageAxes = (function () {
    var AxesObj = function () {
        ...
        this.pixelToLiveString = function (pxi, pyi) {
            var dataVal = this.pixelToData(pxi, pyi);
            return niceNumber(dataVal[0]) + ', ' + niceNumber(dataVal[1]);
        };
...
wpd.TernaryAxes = (function () {
    var AxesObj = function () {
        ...
        this.pixelToLiveString = function (pxi, pyi) {
            var dataVal = this.pixelToData(pxi, pyi);
            return niceNumber(dataVal[0]) + ', ' + niceNumber(dataVal[1]) + ', ' + niceNumber(dataVal[2]);
        };
    ...
    var AxesObj = function () {
        ...
        this.pixelToLiveString = function(pxi, pyi) {
            ...
            if(isXDate) {
                ...
                rtnString += niceNumber(dataVal[0]);
            }
            rtnString += ', ';
            ...
            if(isYDate) {
                ...
                rtnString += niceNumber(dataVal[1]);
            }
            return rtnString;
        };
...
wpd.dataTable = (function () {
    ...
    function setupControls() {
        ...
        if (dataCache.fields.length === 2 && dataCache.fields[0] === 'Label' && dataCache.fields[1] === 'Value') {
            sortingHTML += '<option value="raw">' + wpd.gettext('raw') + '</option>';
            document.getElementById('data-number-format-separator').setAttribute('value', '\\t');
        }
    ...
    function makeTable() {
        ...
        var $digitizedDataTable = document.getElementById('digitizedDataTable'),
            numFormattingDigits = [
                parseInt(document.getElementById('data-number-format-digits-x').value, 10),
                parseInt(document.getElementById('data-number-format-digits-y').value, 10),
                3,
                3
            ],
            numFormattingStyle = [
                document.getElementById('data-number-format-style-x').value,
                document.getElementById('data-number-format-style-y').value,
                'precision',
                'precision'
            ],
            colSeparator = document.getElementById('data-number-format-separator').value,
            showbars = document.getElementById('data-show-bars').checked,	        
            rowi,
            coli,
            rowValues,
            shouldShift = false,
        ...
        for(rowi = 0; rowi < sortedData.length; rowi++) {
            ...
            for(coli = 0; coli < dataCache.fields.length; coli++) {
                ...
                if(dataCache.fieldDateFormat[coli] != null) { // Date
                    ...
                    if(typeof sortedData[rowi][coli] === 'string') {
                        ...
                        if(numFormattingStyle[coli] === 'fixed' && numFormattingDigits[coli] >= 0) {
                            rowValues[coli] = sortedData[rowi][coli].toFixed(numFormattingDigits[coli]);
                        } else if(numFormattingStyle[coli] === 'precision' && numFormattingDigits[coli] >= 0) {
                            rowValues[coli] = sortedData[rowi][coli].toPrecision(numFormattingDigits[coli]);
                            if(rowValues[coli] >= Math.pow(10,numFormattingDigits[coli]) && 
                                    rowValues[coli] <= Math.pow(10,numFormattingDigits[coli] + 3))  {
                                rowValues[coli]=Math.round(rowValues[coli]);
                            }
                        } else if(numFormattingStyle[coli] === 'exponential' && numFormattingDigits[coli] >= 0) {
                            rowValues[coli] = sortedData[rowi][coli].toExponential(numFormattingDigits[coli]);
                        } else {
                            rowValues[coli] = sortedData[rowi][coli];
                        }
                    }
            ...
            if (shouldShift) rowValues.shift();
...
wpd.zoomView = (function() {
    ...
    function showSettingsWindow() {
        ...
        document.getElementById('data-point-size').value = pointRadius;
        document.getElementById('alpha-channel').value = alphaChannel;
        ...
        this.onRedraw = function () {
            ...
            for(i = 0; i < _calibration.getCount(); i++) {
                ...
                if(_calibration.isPointSelected(i)) {
                    fillStyle = "rgba(0,200,0,"+alphaChannel+")";
                } else {
        		    fillStyle = "rgba(200,0,0,"+alphaChannel+")";
                }
...
wpd.graphicsHelper = (function () {
    ...
    function drawPoint(imagePx, fillStyle, label, position) {
        ...
        ctx.dataCtx.arc(screenPx.x, screenPx.y, pointRadius, 0, 2.0*Math.PI, true);
        ctx.dataCtx.fill();
        ctx.dataCtx.stroke();
        ...
        ctx.oriDataCtx.arc(imagePx.x, imagePx.y, pointRadius, 0, 2.0*Math.PI, true);
        ctx.oriDataCtx.fill();
    ...
    var Tool = function (axes, dataset) {
        ...
        this.onMouseClick = function (ev, pos, imagePos) {
            ...
            if(axes.dataPointsHaveLabels) { // e.g. Bar charts
                ...
                wpd.graphicsHelper.drawPoint(imagePos, "rgba(200,0,0,"+alphaChannel+")", pointLabel);

            } else {

                dataset.addPixel(imagePos.x, imagePos.y);
                wpd.graphicsHelper.drawPoint(imagePos, "rgba(200,0,0,"+alphaChannel+")");
    ...
    var Tool = function () {
        var isDrawing = false,
            ...
            mouseUpHandler = function (ev, pos, imagePos) {
                ...
                isDrawing = false;
                wpd.graphicsWidget.resetHover();
                ctx.dataCtx.fillStyle = "rgba(255,255,0,0.7)";
                ctx.dataCtx.fillRect(topScreenCorner.x, topScreenCorner.y, pos.x-topScreenCorner.x, pos.y-topScreenCorner.y);
...
wpd.PenMaskTool = (function () {
    var Tool = function () {
        var strokeWidth,
            ...
            mouseMoveHandler = function() {
                ctx.dataCtx.strokeStyle = "rgba(255,255,0,0.7)";
...
wpd.EraseMaskTool = (function () {
    var Tool = function() {
        ...
        this.onMouseDown = function(ev, pos, imagePos) {
            ...
            ctx.dataCtx.strokeStyle = "rgba(0,0,0,0.7)";
            ctx.dataCtx.lineWidth = lwidth*wpd.graphicsWidget.getZoomRatio();
            ctx.dataCtx.beginPath();
            ctx.dataCtx.moveTo(pos.x,pos.y);
    ...
    var Painter = function () {
        ...
        let painter = function () {
                ...
                for(let img_index of autoDetector.mask) {
                    ...
                    imgData.data[img_index*4+1] = 255;
                    imgData.data[img_index*4+2] = 0;
                    imgData.data[img_index*4+3] = 0.7*255;
                }
...
wpd.alignAxes = (function () {
    ...
    function alignCorners() {
        var p0 = calibration.getPoint(0);
        var p1 = calibration.getPoint(1);
        var p2 = calibration.getPoint(2);
        var p3 = calibration.getPoint(3);
        if (p2) {
            calibration.changePointPx(1, p1.px, p0.py);
            calibration.changePointPx(3, p2.px, p3.py);
        } else {
            if (Math.abs(p1.px - p0.px) < Math.abs(p1.py - p0.py)) {
                calibration.changePointPx(1, p0.px, p1.py);
            } else {
                calibration.changePointPx(1, p1.px, p0.py);
            }
        }
    ...
    function paintFilteredColor(binaryData, maskPixels) {
        ...
        for(let img_index of maskPixels) {
            ...
            if(binaryData.has(img_index)) {
                ...
                dataLayer.data[img_index*4+3] = 178; //0.7*255;
            } else {
